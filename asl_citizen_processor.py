...
Run:
    ...
    python3 asl_citizen_processor.py --glosses ME YOU WE THAT WHY --output selected_words_processed
"""
...
class ASLCitizenProcessor:
    ...
    def __init__(
        ...
        min_frames:       int = 10,
        max_videos:       int = None,
        selected_glosses: list = None,   # explicit list of glosses to process
        output_dir:       str = None,    # output directory (default: asl_citizen_processed)
    ):
        self.src_dir    = Path(src_directory)
        self.min_frames = min_frames
        self.max_videos = max_videos

        # output dir: use --output if provided, else default
        if output_dir:
            self.tgt_dir = Path(output_dir)
        else:
        ...
        # decide which glosses to keep — selected_glosses takes priority
        counts = df["gloss"].value_counts()
        if selected_glosses is not None:
            top = selected_glosses
            df  = df[df["gloss"].isin(top)].copy()
            print(f"Keeping {len(top)} selected glosses ({len(df)} videos)")
            missing = [g for g in selected_glosses if g not in counts.index]
            if missing:
                print(f"  ⚠ Not found in dataset: {', '.join(missing)}")
        elif top_n_glosses is not None:
            ...
            for split_name in ["train", "val", "test"]:
                ...
                for _, row in tqdm(split_df.iterrows(), total=len(split_df)):
                    ...
                    if gloss not in self.mp4_example_by_gloss:
                        ...
                        dest = self.mp4_examples_dir / example_name
        ...
        pd.DataFrame(records).to_csv(self.tgt_dir / "glosses.csv", index=False)
...
if __name__ == "__main__":
    ...
    p.add_argument("--glosses",    nargs="+", default=None,
                   help="Explicit list of glosses to process e.g. ME YOU WE")
    p.add_argument("--output",     default=None,
                   help="Output directory (default: asl_citizen_processed)")
    args = p.parse_args()
